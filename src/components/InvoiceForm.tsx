...
const OPTIONAL_FIELDS = [
  { key: 'fiscalId', label: 'Fiscal Identification Number' },
  { key: 'staticId', label: 'Static Identification Number' },
  { key: 'artistCardNumber', label: 'Artist Card Number' },
  { key: 'bankAccount', label: 'Bank Account' },
  { key: 'phoneNumber', label: 'Phone Number' },
  { key: 'businessRegNumber', label: 'Business Register Number' },
] as const;
...
export default function InvoiceForm({ initialData, onSubmit, onCancel }: Props) {
  ...
  const renderOptionalFieldsSection = (entity: 'company' | 'client') => {
    ...
    const availableFields = OPTIONAL_FIELDS.filter(({ key }) => !fields[key as keyof OptionalFields]);
          ...
          <div key={key} className="flex items-center gap-4">
            <div className="flex-grow">
              <label className="block text-sm font-medium text-gray-700">
                {OPTIONAL_FIELDS.find(f => f.key === key)?.label}
              </label>
        ...
        {availableFields.length > 0 && (
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="mt-1 inline-flex items-center p-2 border border-transparent rounded-md shadow-sm text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500"
            >
